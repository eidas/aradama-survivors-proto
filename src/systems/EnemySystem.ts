 ...
 * ボス(E6 大型集合体)専用の AI は BossBehavior に分離している。
 */
export class EnemySystem {
  ...
  constructor(private game: GameScene) {
    this.bossBehavior = new BossBehavior(game);
  }
  ...
  cancelTelegraph(e: Enemy): void {
    ...
    if (e.def.ai === 'charge') {
      ...
      this.bossBehavior.cancelTelegraph(e);
    }
  }
  ...
  private updateOrbitDive(e: Enemy, dt: number): void {
    ...
    if (e.aiState === 0) {
      ...
      moveToward(e, tx, ty, e.speed * 1.5, dt);
      if (e.aiTimer <= 0) {
        e.aiState = 1;
